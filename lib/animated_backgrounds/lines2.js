...
export default function lines2({canvas, is_phone, safeAddEventListener, darkmode, getwinsize, isVisible}){
    ...
    const draw = ()=>{
        ...
        for(let x=.5*SIZE+padX; x<can.width-.5*SIZE; x += SIZE){
            for(let y=.5*SIZE+padY; y<can.height-.5*SIZE; y += SIZE*(1+window.scrollY*.001)){
                ...
                ctx.lineWidth = (1-window.scrollY*.0025)*((6+10)+10*noise3D(7+x*noise_scale*1.3, 4+y*noise_scale*1.7, 10+T*.00017));
