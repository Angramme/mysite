...
class Dancer{
    ...
    draw(can, ctx, t){
        ...
        const scsc = Math.max(0, 1-window.scrollY*.0035);
        this._draw(ctx, 
...
export default function dancing_shapes({canvas, is_phone, safeAddEventListener, darkmode, getwinsize, isVisible}){
    ...
    const draw = ()=>{
        ...
        if(trace_cnt > 3){
            ...
            ctx2.globalAlpha = .9 - Math.min(0.8, window.scrollY*0.002);
